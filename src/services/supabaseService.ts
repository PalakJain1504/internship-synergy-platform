...
export const fetchInternships = async (): Promise<InternshipData[]> => {
  try {
    ...
    const formattedData: InternshipData[] = data.map(item => ({
      ...
      dates: item.dates || '',
      internshipDuration: item.internship_duration || '',
  ...
  try {
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        internship_duration: formattedInternship.internshipDuration,
        mobile_number: formattedInternship.mobileNumber,
...
export const uploadMultipleInternships = async (internships: InternshipData[]): Promise<{ success: boolean, error?: any }> => {
  try {
    ...
    const formattedInternships = internships.map(internship => ({
      ...
      internship_duration: internship.internshipDuration,
      mobile_number: internship.mobileNumber,
      noc: internship.noc,
